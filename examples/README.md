...
to your `.env`, or wherever `DISTILL_CACHE_DIR` points. "panel" below means the
export at `cache/panel.csv`, or the published 200-firm sample at
`cache/pit-sample.csv.gz` when that is what is on disk (README, Install).
...
| `fetch_panel.py` | free key (current snapshot) or Pro (full history, about 80 MB CSV) | one call for the screener panel as CSV |
